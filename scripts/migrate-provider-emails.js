 ...
 */
async function migrateProviderEmails(defaultProviderEmail) {
  try {
    if (!defaultProviderEmail) {
      console.error('Please provide a default provider email as an argument.');
      console.log('Usage: node migrate-provider-emails.js provider@example.com');
      process.exit(1);
    }
    ...
    console.log(`Found ${patients.length} patients without a provider email assignment`);
