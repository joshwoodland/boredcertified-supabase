...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!settings) {
      debugLog('No settings found in Supabase, using memory defaults');
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // If we couldn't update settings with Supabase, return an error or use in-memory defaults
    if (!updatedSettings) {
      debugLog('Supabase unavailable, using in-memory defaults');

      // Return a mock result with the requested changes
      updatedSettings = {
        id: 'default',
