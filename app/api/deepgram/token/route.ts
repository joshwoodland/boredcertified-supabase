  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        return NextResponse.json(
          {
            error: 'Deepgram API access denied',
            details: 'The API key does not have permission to create temporary keys',
            responseText: errorText
          },
          { status: 403 }
        );
    ...
    console.log('[DEEPGRAM TOKEN API] Successfully generated streaming token');

    // Return the token and expiration time to the client
    return NextResponse.json({
      token: data.key,
      expiresIn: requestedTtl, // Use the requested TTL
      scopes: ['usage:write', 'usage:read'], // Include the scopes for debugging
      projectId: projectId
    });
  } catch (error) {
    // Log the error
